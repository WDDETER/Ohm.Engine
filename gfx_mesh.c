...
void mesh_wireframe(struct painter* painter, struct mesh* mesh, mat4f mvp, color32i wireframe_color)
{
        ...
        for (int i = 0; i < mesh->indices_size; i += 3)
        {
                ...
                vec2f edge1 = { .x = vertex2.position.x - vertex1.position.x, .y = vertex2.position.y - vertex1.position.y };
                vec2f edge2 = { .x = vertex3.position.x - vertex1.position.x, .y = vertex3.position.y - vertex1.position.y };

                float cross_z = vec2f_cross(edge1, edge2);
...
//      !NOTICE: Not very clean, need to refactor.
enum gfx_error mesh_hire_obj(struct mesh* mesh, const char* file_path)
{
        ...
        if (!file)
                return MESH_FAILED_TO_READ_OBJ;


        char line[2048];        // definitely don't want to hardcode this.
        ...
        while (fgets(line, sizeof(line), file) != NULL)
        {
                ...
                if (line[0] == 'f' && line[1] == ' ') indices_count++;

        }

        mesh->vertices_count = vertices_count;
        mesh->indices_height = indices_count;
        ...
        while (fgets(line, sizeof(line), file) != NULL)
        {
                ...
                if (line[0] == 'v' && line[1] == ' ')
                {
                        ...
                        mesh->vertices[vertices_idx].position.x = x;
                        mesh->vertices[vertices_idx].position.y = y;
                        mesh->vertices[vertices_idx].position.z = z;
                        mesh->vertices[vertices_idx].position.w = 1.0f;
                ...
                if (line[0] == 'f' && line[1] == ' ')
                {
                        ...
                        if (parsed < 3)
                        {
                                ...
                                parsed = sscanf(line, "f %d %d %d", &i1, &i2, &i3);
                        ...
                        if (parsed >= 3)
                        {
                                ...
                                mesh->indices[indices_idx++] = i1 - 1;
                                mesh->indices[indices_idx++] = i2 - 1;
                                mesh->indices[indices_idx++] = i3 - 1;
